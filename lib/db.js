...
export async function updateJob(id, updates) {
  const { status, step, outputUrl, error, completedAt } = updates;
...
// Template Jobs CRUD
export async function createTemplateJob({ name, pipeline, videoSource, tiktokUrl, videoUrl }) {
  const totalSteps = pipeline.filter(s => s.enabled).length;
  const result = await sql`
    INSERT INTO template_jobs (name, pipeline, total_steps, video_source, tiktok_url, video_url)
    VALUES (${name}, ${JSON.stringify(pipeline)}, ${totalSteps}, ${videoSource || 'tiktok'}, ${tiktokUrl || null}, ${videoUrl || null})
    RETURNING *
  `;
...
export async function updateTemplateJob(id, updates) {
  const { status, currentStep, step, outputUrl, error, completedAt } = updates;
  const result = await sql`
